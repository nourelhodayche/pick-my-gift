        ...
        <div style={{ marginTop: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {isOwner ? (
            ...
            <button
              ...
              style={{
                marginLeft: "auto", display: "flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 600,
                ...
                borderRadius: 8, padding: "5px 12px", cursor: isReserved && !isReservedByMe ? "not-allowed" : "pointer",
  ...
  const handleAdd = async () => {
    ...
    try {
      await api.post(`/api/gifts/event/${eventId}`, {
        name: preview.data?.title || preview.title,
        image: preview.image,
        price: preview.price,
        url: preview.url || url,
      });
          ...
          <div style={{ border: "1.5px solid #EEF1F6", borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
            <div style={{ display: "flex", gap: 14, padding: 14, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {preview.price && <p style={{ margin: 0, fontWeight: 700, color: "#2563FF", fontSize: 15 }}>{preview.price} MAD</p>}
              </div>
            </div>
          </div>
        )}
...
setMyReservations(resRes.data);
  ...
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
        console.log('LOAD STARTED, id:', id)
      try {
        const [eventRes, giftsRes, resRes] = await Promise.all([
  api.get(`/api/events/${id}`),
  api.get(`/api/gifts/event/${id}`),
 ...
 console.log('GIFTS:', giftsRes.data)
...
setMyReservations(resRes.data);
  ...
  const handleReserve = async (giftId) => {
    ...
    try {
      ...
      alert(e.message || "Could not reserve this gift.");
    } finally {
      setReservingId(null);
    }
  };
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <main style={{ flex: 1, padding: "28px 32px", maxWidth: 1100, width: "100%" }}>
          ...
          <div>
            ...
            {gifts.length === 0 ? (
              <div style={{
                background: "#fff", border: "1.5px dashed #D0D5DD", borderRadius: 20,
                padding: "48px 24px", textAlign: "center"
              }}>
                <div style={{ color: "#D0D5DD", marginBottom: 12 }}>
                  <IconGift />
                </div>
                ...
                <p style={{ margin: "0 0 20px", color: "#98A2B3", fontSize: 13 }}>
                  {isOwner ? "Add gifts by pasting product links below." : "The organizer hasn't added any gifts yet."}
                </p>
                {isOwner && (
                  <button onClick={() => setShowAddModal(true)}
